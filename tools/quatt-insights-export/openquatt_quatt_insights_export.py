...
DAILY_REQUIRED_IMPORT_KEYS = (
    "energy_hp_electric",
    "energy_hp_heat",
)
DAILY_SUMMARY_IMPORT_KEYS = {
    "totalHpElectric": "energy_hp_electric",
    "totalHpHeat": "energy_hp_heat",
    "totalBoilerHeat": "energy_boiler_heat",
}
MAX_MONTH_SOURCE_CALLS = 120
MAX_DAY_SOURCE_CALLS = 366
...
def _get_response_data(response):
    ...
    if isinstance(response, dict) and response.get("error"):
        raise RuntimeError(f"quatt.get_cic_insights failed: {response['error']}")
    return response
...
def _normalise_day_from_totals(data, day_date, include_raw):
    data = data if isinstance(data, dict) else {}

    day = {"date": day_date}
    for source_key, target_key in DAILY_SUMMARY_IMPORT_KEYS.items():
        if source_key in data:
            day[target_key] = data[source_key]
...
def _missing_days(days, start_date, end_date):
    available_days = {
        day.get("date")
        for day in days
        if all(key in day for key in DAILY_REQUIRED_IMPORT_KEYS)
    }
    return [
        _format_date(day)
...
def _local_date_from_timestamp(timestamp, local_timezone):
    ...
    try:
        ...
        return parsed.astimezone(ZoneInfo(local_timezone)).date().isoformat()
    except Exception:
...
def _validate_timezone(local_timezone):
    try:
        ZoneInfo(local_timezone)
    except Exception as err:
        raise ValueError(f"Unknown local_timezone: {local_timezone}") from err


def _validate_delay_ms(day_delay_ms):
    try:
        delay_ms = float(day_delay_ms)
    except (TypeError, ValueError) as err:
        raise ValueError("day_delay_ms must be a number from 0 through 5000") from err
    if not 0 <= delay_ms <= 5000:
        raise ValueError("day_delay_ms must be a number from 0 through 5000")
    return delay_ms


def _validate_call_count(count, limit, source):
    if count > limit:
        raise ValueError(
            f"daily_source {source} would make {count} API calls; "
            f"split the export into ranges of at most {limit} calls"
        )
...
def _inclusive_month_count(start_date, end_date):
    return (
...
def _write_export_files(output_dir, base_name, payload):
    ...
    try:
        ...
        os.close(json_fd)
        csv_fd, tmp_csv_path = tempfile.mkstemp(
            dir=output_dir,
            prefix=f".{safe_base}.",
            suffix=".csv.tmp",
        )
        ...
        for tmp_path in (tmp_json_path, tmp_csv_path):
            if not tmp_path:
                continue
            try:
                os.unlink(tmp_path)
...
description: Export Quatt CiC insights as JSON and CSV. Daily mode uses the OpenQuatt import schema.
fields:
  from_date:
  ...
  to_date:
    ...
    selector:
      date:
  timeframe:
    description: Quatt insights timeframe when daily mode is disabled.
    default: all
    selector:
      select:
        options:
...
"""
    ...
    delay_ms = _validate_delay_ms(day_delay_ms)
    ...
    if daily:
        ...
        end_date = _parse_date(to_date) if to_date else _today(local_timezone)
        if end_date < start_date:
            raise ValueError("to_date must be equal to or after from_date")
        today = _today(local_timezone)
        request_end_date = min(end_date, today)

        if daily_source == "month":
            days_by_date = {}
            if request_end_date >= start_date:
                _validate_call_count(
                    _inclusive_month_count(start_date, request_end_date),
                    MAX_MONTH_SOURCE_CALLS,
                    daily_source,
                )
                month_starts = _month_range(start_date, request_end_date)
            else:
                month_starts = []
            ...
            for index, month_start in enumerate(month_starts):
                ...
                if delay_ms and index < len(month_starts) - 1:
                    task.sleep(delay_ms / 1000.0)
            ...
            if request_end_date >= start_date:
                _validate_call_count(
                    _inclusive_day_count(start_date, request_end_date),
                    MAX_DAY_SOURCE_CALLS,
                    daily_source,
                )
                day_dates = _date_range(start_date, request_end_date)
            else:
                day_dates = []
            for day_date in day_dates:
                ...
                day = _normalise_day_from_totals(
                ...
                if all(key in day for key in DAILY_REQUIRED_IMPORT_KEYS):
                    days.append(day)
                if delay_ms and day_date < request_end_date:
                    task.sleep(delay_ms / 1000.0)
